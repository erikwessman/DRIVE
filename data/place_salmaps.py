    ...
    for accid in sorted(os.listdir(coord_path)):
        ...
        for filename in sorted(os.listdir(coord_file_path)):
            ...
            if not os.path.exists(salmap_video_src):
                print(
                    "salmap video file does not exist! use focus map instead! %s"
                    % (salmap_video_src)
                )
                salmap_video_src = os.path.join(
                    data_root, subset, "focus_videos", accid, vid + ".avi"
                )
                assert os.path.exists(
                    salmap_video_src
                ), "video file does not exist! %s" % (salmap_video_src)
            ...
            # copy file
            shutil.copyfile(
                salmap_video_src, os.path.join(salmap_dst_path, vid + ".avi")
            )
