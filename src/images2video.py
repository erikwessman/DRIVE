...
def read_video_frames(video_path):
    ...
    ):  # we must sort the image files to ensure a sequential frame order
        # get frame id list
...
if __name__ == "__main__":
    ...
    for phase in ["training", "testing", "validation"]:
        ...
        for index, dataID in tqdm(
            enumerate(data_list), desc=phase, total=len(data_list)
        ):
            # video path
            ...
            result_video_path = os.path.join(
                root_path, phase, "rgb_videos", data_list[index].split("/")[0]
            )
            if not os.path.exists(result_video_path):
                os.makedirs(result_video_path)
            video_file = os.path.join(
                result_video_path, data_list[index].split("/")[1] + ".avi"
            )
            # result focus path
            result_focus_path = os.path.join(
                root_path, phase, "focus_videos", data_list[index].split("/")[0]
            )
            if not os.path.exists(result_focus_path):
                os.makedirs(result_focus_path)
            focus_file = os.path.join(
                result_focus_path, data_list[index].split("/")[1] + ".avi"
            )
            ...
            # write focus video
            focus_writer = cv2.VideoWriter(
                focus_file,
                cv2.VideoWriter_fourcc(*"DIVX"),
                fps,
                (focus_data[0].shape[1], focus_data[0].shape[0]),
            )
