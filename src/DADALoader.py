...
class DADALoader(Dataset):
    ...
    def read_frames_from_videos(self, index, interval=1, max_frames=-1):
        ...
        video_path = os.path.join(
            self.root_path, self.phase, "rgb_videos", self.data_list[index] + ".avi"
        )
        assert os.path.exists(video_path), "Path does not exist: %s" % (video_path)
        frame_ids, video_data = [], []
        # get the video data
        ...
        if max_frames > 0:
            ...
            video_data = [video_data[i] for i in inds]
        video_data = np.array(
            video_data, dtype=np.float32
        )  # 4D tensor, (N, 660, 1584, 3)
        return video_data, frame_ids
    ...
    def read_focus_from_videos(self, frame_ids, index):
        ...
        focus_path = os.path.join(
            self.root_path, self.phase, "focus_videos", self.data_list[index] + ".avi"
        )
        assert os.path.exists(focus_path), "Path does not exist: %s" % (focus_path)
        focus_data = []
    ...
    def read_coord_arrays(self, frame_ids, index):
        ...
        coord_file = os.path.join(
            ...
            self.phase,
            "coordinate",
            self.data_list[index] + "_coordinate.txt",
        )
    ...
    def gather_info(self, index, video_data):
        ...
        data_info = np.array(
            [accident_id, video_id, nframes, height, width], dtype=np.int64
        )
        return data_info
    ...
    def __getitem__(self, index):
        ...
        video_data, frame_ids = self.read_frames_from_videos(
            index, interval=self.interval, max_frames=self.max_frames
        )
        ...
        if self.transforms["image"] is not None:
            ...
            if self.params_norm is not None:
                for i in range(video_data.shape[1]):
                    video_data[:, i] = (
                        video_data[:, i] - self.params_norm["mean"][i]
        ...
        if self.cls_task:
            data_input, label_target, logit_target = self.pre_process(
                video_data.astype(np.float32), coord_data, data_info
            )
...
class PreFetcher:
    ...
    def preload(self):
        try:
            self.next_video_data, self.next_focus_data, self.next_coord_data = next(
                self.loader
            )
    ...
    def next(self):
        ...
        next_video_data, next_focus_data, next_coord_data = (
            self.next_video_data,
            self.next_focus_data,
            self.next_coord_data,
        )
        self.preload()
...
def setup_dataloader(cfg):
    transform_dict = {
        "image": transforms.Compose([ProcessImages(cfg.input_shape)]),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_path", default="./data/DADA-2000", help="The relative path of dataset."
    )
    parser.add_argument(
        "--batch_size",
        type=int,
    ...
    for i, (video_data, _, coord_data, data_info) in enumerate(traindata_loader):
        ...
        print(
            "batch: %d / %d, num_frames = %d, cls = %d, time=%.3f"
            % (
                i,
