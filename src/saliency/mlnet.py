...
class ResNet_FPN(torch.nn.Module):
    ...
    def forward(self, im):
        ...
        p4 = (
            F.interpolate(p5, size=(c4_lat.size(2), c4_lat.size(3)), mode="bilinear")
            + c4_lat
        )
        ...
        p3 = (
            F.interpolate(p4, size=(c3_lat.size(2), c3_lat.size(3)), mode="bilinear")
            + c3_lat
        )
        ...
        p2 = (
            F.interpolate(p3, size=(c2_lat.size(2), c2_lat.size(3)), mode="bilinear")
            + c2_lat
        )
...
class ModMSELoss(torch.nn.Module):
    ...
    def forward(self, output, label, prior):
        ...
        output_max = (
            torch.max(torch.max(output, 2)[0], 2)[0]
            .unsqueeze(2)
            .unsqueeze(2)
            .expand(output.shape[0], output.shape[1], self.shape_r_gt, self.shape_c_gt)
        )
        reg = (1.0 / (prior_size[0] * prior_size[1])) * (1 - prior) ** 2  # (1, 1, 6, 8)
        loss = torch.mean(
            ((output / (output_max + 1e-6) - label) / (1 - label + 0.1)) ** 2
        ) + torch.sum(reg)
        return loss
