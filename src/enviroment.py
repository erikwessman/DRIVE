...
class DashCamEnv(core.Env):
    def __init__(self, cfg, device=torch.device("cuda")):
        ...
        self.foveal_model = TorchFovea(
            cfg.input_shape,
            min(cfg.input_shape) / 6.0,
            level=5,
            factor=2,
            device=device,
        )
    ...
    def set_model(self, pretrained=False, weight_file=None):
        if pretrained and weight_file is not None:
            ...
            assert os.path.exists(
                weight_file
            ), "Checkpoint directory does not exist! %s" % (weight_file)
                ...
                for name, param in ckpt.items():
                    ...
                    if name in model_dict:
                        if param.size() == model_dict[name].size():
                            ...
                            print(
                                " size? " + name, param.size(), model_dict[name].size()
                            )
                    else:
                        print(" name? " + name)
    ...
    def set_data(self, video_data, coord_data, data_info):
        ...
        self.begin_accident = data_info[:, 5].to(self.device) / float(
            self.fps
    ...
    def reset(self):
        ...
        # fetch video clip
        frame_data = self.video_data[
            :,
            self.cur_step * self.step_size : self.cur_step * self.step_size
            + self.len_clip,
        ]  # (B, T, C, H, W)
        # observation by computing saliency
        with torch.no_grad():
            self.cur_state = self.observe(
                frame_data, fixation=None
            )  # GPU Array(B, 124)
        ...
        if self.saliency == "MLNet":
            ...
            if fixation is not None:
                ...
                rho = (
                    ...
                    else self.rho.unsqueeze(1).unsqueeze(1).unsqueeze(1)
                )  # (B, 1, 1, 1)
            ...
            # construct states
            state_max = (
                F.max_pool2d(saliency * bottom, kernel_size=bottom.size()[2:])
                .squeeze_(dim=-1)
                .squeeze_(dim=-1)
            )
            state_avg = (
                F.avg_pool2d(saliency * bottom, kernel_size=bottom.size()[2:])
                .squeeze_(dim=-1)
                .squeeze_(dim=-1)
            )
        elif self.saliency == "TASED-Net":
            data_observe = data_input.permute(
                0, 2, 1, 3, 4
            ).contiguous()  # (B, C=3, T=32, H=480, W=640)
            # compute saliency map
            saliency_bu, bottom = self.observe_model(
                data_observe, return_bottom=True
            )  # (1,1,1,60,80), [1, 192, 4, 60, 80]
            ...
            if fixation is not None:
                ...
                saliency_td = self.observe_model(
                    data_foveal.unsqueeze(2).repeat(1, 1, T, 1, 1)
                )  # (B, 1, 1, 60, 80)
                ...
                rho = (
                    self.rho
                    if self.fusion == "static"
                    else self.rho.unsqueeze(1).unsqueeze(1).unsqueeze(1).unsqueeze(1)
                )  # (B, 1, 1, 1, 1)
                saliency = (1 - rho) * saliency_bu + rho * saliency_td
            else:
                saliency = saliency_bu.clone()
            # construct states
            state_max = (
                F.max_pool3d(saliency * bottom, kernel_size=bottom.size()[2:])
                .squeeze_(dim=-1)
                .squeeze_(dim=-1)
                .squeeze_(dim=-1)
            )
            state_avg = (
                F.avg_pool3d(saliency * bottom, kernel_size=bottom.size()[2:])
                .squeeze_(dim=-1)
                .squeeze_(dim=-1)
                .squeeze_(dim=-1)
            )
    ...
    def get_next_state(self, fixation_next, next_step):
        """mask_pred: (B, 2)"""
        frame_data = self.video_data[
            :, next_step * self.step_size : next_step * self.step_size + self.len_clip
        ]  # (B, T, C, H, W)
        with torch.no_grad():
    ...
    def get_reward(self, score_pred, fix_pred):
        ...
        exp_term = torch.clamp(
            self.begin_accident - self.cur_step * self.step_size / self.fps, min=0
        )  # max(0, ta-t)
        ...
        fix_gt = self.coord_data[
            :, (self.cur_step + 1) * self.step_size, :
        ]  # (B, 2), [x, y]
        # compute distance
        dist_sq = torch.sum(
            torch.pow(
        ...
        r_atten = mask_reward.unsqueeze(1) * torch.exp(
            -10.0 * dist_sq
    ...
    def step(self, actions, isTraining=True):
        ...
        fix_pred = scales_to_point(
            actions[:, 1:], self.image_size, self.input_size
        )  # (B, 2)  (x,y)
        ...
        if self.cur_step < self.max_steps - 1:  # cur_step starts from 0
            ...
            cur_rewards = (
                torch.zeros([batch_size, 1], dtype=torch.float32).to(self.device)
                if isTraining
                else 0
            )
