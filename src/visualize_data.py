...
if __name__ == "__main__":
    ...
    salmap_file = os.path.join(
        root_path, phase, "salmap_videos", atype, sequence + ".avi"
    )
    coord_file = os.path.join(
        root_path, phase, "coordinate", atype, sequence + "_coordinate.txt"
    )
    ...
    for t, (frame, salmap, fixation) in enumerate(
        zip(video_data, salmap_data, coord_data)
    ):
        ...
        if fixation[0] != 0 and fixation[1] != 0:
            ...
            progress_bar[
                :, t * step : min((t + 1) * step, w), 0
            ] = 0  # fill red color (0, 0, 255)
        ...
        visframe[
            frame.shape[0] - barWidth - 1 : frame.shape[0] - 1, :, :
        ] = cv2.addWeighted(
            visframe[frame.shape[0] - barWidth - 1 : frame.shape[0] - 1, :, :],
            0.3,
            progress_bar,
            0.7,
            0,
