...
class DADA2KS(Dataset):
    ...
    def gather_info(self, index):
        ...
        if self.labels[index] > 0:  # positive sample
            assert (
                self.toas[index] >= start and self.toas[index] <= end
            ), "sample id: %s" % (self.data_list[index])
            toa = int((self.toas[index] - start) / self.interval)
        else:
            toa = int(self.toas[index])  # negative sample (toa=-1)
        data_info = np.array(
            [accident_id, video_id, start, end, self.labels[index], toa], dtype=np.int32
        )
        return data_info
    ...
    def __getitem__(self, index):
        ...
        if self.use_salmap:
            salmap_path = os.path.join(
                self.root_path,
                self.phase,
                "salmap_videos",
                self.data_list[index] + ".avi",
            )
...
if __name__ == "__main__":
    ...
    for i, (video_data, _, coord_data, data_info) in tqdm(
        enumerate(traindata_loader), total=len(traindata_loader), desc="[training set]"
    ):
        # compute mean
    ...
    for i, (video_data, _, coord_data, data_info) in tqdm(
        enumerate(evaldata_loader), total=len(evaldata_loader), desc="[validation set]"
    ):
        # compute mean
        video_data = video_data.to(device, non_blocking=True)  # (B, T, C, H, W)
        mean_vec += torch.mean(video_data, (0, 1, 3, 4))
