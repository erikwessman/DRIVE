...
class TorchFovea(torch.nn.Module):
    def __init__(
        self, imgsize, sigma, level=5, factor=2.0, device=torch.device("cuda")
    ):
        ...
        self.filter_sizes = [
            torch.Tensor([self.filters[i].size(1), self.filters[i].size(0)]).to(device)
    ...
    def createFilter(self, height, width):
        ...
        [ex, ey] = torch.meshgrid(
            torch.arange(-xc, width - xc - 1).to(device=self.device),
    ...
    def create_pyramid_filters(self):
        ...
        for i in range(self.level):
            kernel_down = (
                PyrDown()(kernels[i].unsqueeze(0).unsqueeze(0)).squeeze(0).squeeze(0)
            )
    ...
    def foveate(self, images, fixations):
        ...
        image_sizes = [
            [image_pyrmid[i].size(3), image_pyrmid[i].size(2)]
            for i in range(self.level)
        ]
        ...
        for i in range(self.level - 1, -1, -1):
            ...
            kernel_rect = [
                self.filters[i][y1[j] : y2[j], x1[j] : x2[j]] for j in range(batchsize)
            ]
            kernel_rect = torch.stack(kernel_rect).unsqueeze(
                1
            )  # expand channel dimension  # (B, 1, 480, 639)
            # filtering
            kernel_rect = F.interpolate(
                kernel_rect, [image_sizes[i][1], image_sizes[i][0]]
            )
            im_filtered = image_pyrmid[i] * kernel_rect.repeat(1, channel, 1, 1)
            if i != self.level - 1:
                fovea_image = PyrUp()(fovea_image)
                fovea_image = F.interpolate(
                    fovea_image, [im_filtered.size(2), im_filtered.size(3)]
                )
            # take summation
...
if __name__ == "__main__":
    ...
    images = torch.stack((torch.from_numpy(im), torch.from_numpy(im))).to(
        device
    )  # (B, C, H, W)
    ...
    t_start = time.time()
    fovea_layer = TorchFovea(
        (height, width), sigma, level=levels, factor=factor, device=device
    )
