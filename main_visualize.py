...
def read_frames_from_videos(
    root_path, vid_name, start, end, folder, phase="testing", interval=1
):
    """Read video frames"""
...
def plot_scores(pred_scores, toa, n_frames, frame_interval, out_file):
    ...
    plt.axhline(
        y=0.5, xmin=0, xmax=n_frames + 1, linewidth=3.0, color="g", linestyle="--"
    )
    if toa >= 0:
        plt.axvline(x=toa, ymax=1.0, linewidth=3.0, color="r", linestyle="--")
        x = [toa, n_frames * frame_interval]
...
def generate_attention(
    ...
    for i in range(n_slice):
        ...
        input_data = torch.FloatTensor(
            data_trans(frame_data[i * slice_size : (i + 1) * slice_size])
        ).to(device)
        fixation_data = torch.from_numpy(
            fixations[i * slice_size : (i + 1) * slice_size]
        ).to(device)
        ...
        rho = np.expand_dims(
            np.expand_dims(
                np.array(rho_list[i * slice_size : (i + 1) * slice_size]), axis=1
            ),
            axis=2,
        )
...
def saliency_padding(saliency, image_size):
    ...
    if rows_rate > cols_rate:
        ...
        patch_ctr = saliency[
            :, ((height - new_rows) // 2) : ((height - new_rows) // 2 + new_rows), :
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_path",
        default="data/DADA-2000-small",
        help="Configuration file for SAC algorithm.",
    )
    parser.add_argument(
        "--sal_ckpt",
        default="models/saliency/mlnet_25.pth",
        help="Pretrained model for bottom-up saliency prediciton.",
    )
    parser.add_argument(
        "--test_results",
        default="output/DADA2KS_Full_SACAE_Final/eval/results.npz",
        help="Result file of testing data.",
    )
    parser.add_argument("--rho", type=float, default=0.5, help="The rho value")
    parser.add_argument("--margin", type=float, default=0.5, help="The margin value")
    parser.add_argument(
        "--static", action="store_true", help="whether to use static fusion test"
    )
    parser.add_argument(
        "--no_curve_overlap",
        action="store_true",
    ...
    parser.add_argument(
        "--output",
        default="./output/DADA2KS_Full_SACAE_Final/vis_results-01152022",
    ...
    assert os.path.exists(args.sal_ckpt), "Checkpoint directory does not exist! %s" % (
        args.sal_ckpt
    )
    ...
    observe_model.eval()
    fovealmodel = TorchFovea(
        (height, width), min(height, width) / 6.0, level=5, factor=2, device=device
    )
    # transform
    data_trans = transforms.Compose(
        [
            ProcessImages(
                (height, width), mean=[0.218, 0.220, 0.209], std=[0.277, 0.280, 0.277]
            )
        ]
    )
    ...
    for i, vids in enumerate(all_vids):
        ...
        # read frames
        frames = read_frames_from_videos(
            args.data_path,
            vidname,
            start,
            end,
        ...
        if not args.no_curve_overlap:
            # create curves
            curve_frames = create_curve_video(
                pred_scores, toa, len(frames), frame_interval
            )
        # plot curves
        ...
        attention_maps = generate_attention(
            frames,
            data_trans,
            observe_model,
            fovealmodel,
        ...
        gt_salmaps = read_frames_from_videos(
            args.data_path,
            vidname,
            start,
            end,
        ...
        video_writer = cv2.VideoWriter(
            ...
            display_fps,
            (image_size[1], height_vis),
        )
        for t, frame in enumerate(frames):
            # add pred_mask as heatmap
            heatmap = cv2.applyColorMap(
                (attention_maps[t] * 255).astype(np.uint8), cv2.COLORMAP_JET
            )
            ...
            if not args.no_curve_overlap:
                ...
                curve_height = int(
                    curve_img.shape[0] * (image_size[1] / curve_img.shape[1])
                )
                curve_img = cv2.resize(
                    curve_img,
                    (image_size[1], curve_height),
                    interpolation=cv2.INTER_AREA,
                )
                frame_vis[
                    image_size[0] - curve_height : image_size[0]
                ] = cv2.addWeighted(
                    frame_vis[image_size[0] - curve_height : image_size[0]],
                    0.3,
                    curve_img,
                    0.7,
                    0,
