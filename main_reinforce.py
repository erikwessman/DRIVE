...
def parse_main_args():
    ...
    parser.add_argument(
        "--config",
        default="cfgs/reinforce_mlnet.yml",
    ...
    parser.add_argument(
        "--output", default="./output/REINFORCE", help="Directory of the output. "
    )
...
def train_per_epoch(traindata_loader, env, agent, cfg, writer, epoch):
    ...
    for i, (video_data, _, coord_data, data_info) in tqdm(
        ...
        desc="Training Epoch: %d / %d" % (epoch + 1, cfg.num_epoch),
    ):  # (B, T, H, W, C)
        ...
        rnn_state = (
            Variable(
                torch.zeros((2, cfg.ENV.batch_size, cfg.REINFORCE.hidden_size))
            ).to(cfg.device)
            if cfg.REINFORCE.use_lstm
            else None
        )
        # run each time step
        episode_steps = 0
        ...
        while episode_steps < env.max_steps:
            ...
            gt_fix_next = env.coord_data[:, next_step * env.step_size, :].unsqueeze(
                1
            )  # (B, 1, 2)
...
def eval_per_epoch(evaldata_loader, env, agent, cfg, writer, epoch):
    ...
    for i, (video_data, _, coord_data, data_info) in tqdm(
        ...
        desc="Testing Epoch: %d / %d" % (epoch + 1, cfg.num_epoch),
    ):  # (B, T, H, W, C)
        # set environment data
        state = env.set_data(video_data, coord_data, data_info)
        rnn_state = (
            torch.zeros((2, cfg.ENV.batch_size, cfg.REINFORCE.hidden_size)).to(
                cfg.device
            )
            if cfg.REINFORCE.use_lstm
            else None
    ...
    for e in range(cfg.num_epoch):
        ...
        if (e + 1) % cfg.snapshot_interval == 0:
            ...
            torch.save(
                ...
                os.path.join(ckpt_dir, "reinforce_epoch_%02d.pth" % (e + 1)),
            )
...
def test_all(testdata_loader, env, agent):
    ...
    for i, (video_data, _, coord_data, data_info) in enumerate(
        ...
        while i_steps < env.max_steps:
            ...
            fixation_pred[:, i_steps] = (
                info["pred_fixation"].cpu().numpy()
            )  # shape=(B, 2)
            next_step = (
