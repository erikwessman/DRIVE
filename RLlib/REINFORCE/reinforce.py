...
class REINFORCE(object):
    def __init__(self, cfg, device=torch.device("cuda")):
        self.policy_model = Policy(cfg.hidden_size, cfg.dim_state, cfg.dim_action).to(
            device
        )
        ...
        if self.with_baseline:
            self.value_estimator = ValueEstimator(cfg.hidden_size, cfg.dim_state, 1).to(
                device
            )
            self.advantage_loss = torch.nn.MSELoss(reduction="none")
            self.value_optimizer = optim.Adam(
                self.value_estimator.parameters(), lr=cfg.lr_adv
            )
    ...
    def update_parameters(
        ...
        for i in reversed(range(horizon)):
            ...
            if self.with_baseline:
                ...
                adv_loss = adv_loss + torch.mean(
                    self.advantage_loss(baseline, Variable(R))
                )
                R = R - baseline.detach()  # advantage
            # total loss
            policy_loss = (
                policy_loss
                - (log_probs[i] * (Variable(R).expand_as(log_probs[i]))).sum()
                - (cfg.REINFORCE.alpha * entropies[i]).sum()
            )

            # get the action with grad
            state = states[i - horizon + 1]
            action, _, _, rnn_state = self.select_action(
                state, rnn_state, with_grad=True
            )  # (B, 1, 3)
        ...
        cls_loss, fix_loss = self.compute_task_loss(
            actions, all_times, all_fixations, env, cfg.ENV
        )
        total_loss = (
            policy_loss
            + cfg.REINFORCE.beta_accident * cls_loss
            + cfg.REINFORCE.beta_fixation * fix_loss
        )
        # update policy model
    ...
    def compute_task_loss(self, actions, all_times, all_fixations, env, cfg):
        ...
        cls_loss = exp_loss(
            score_pred.view(-1), cls_target, all_times.view(-1), toa_batch
        ...
        fix_pred = scales_to_point(
            fix_pred, cfg.image_shape, cfg.input_shape
        )  # scaling scales to point
        fix_loss = torch.sum(
            torch.pow(
                norm_fix(fix_pred, cfg.input_shape) - norm_fix(fix_gt, cfg.input_shape),
                2,
            ),
            dim=1,
        ).mean()  # (B) [0, sqrt(2)]
    ...
    def load_models(self, ckpt_dir, cfg):
        if cfg.test_epoch == -1:
            ...
            weight_file = os.path.join(
                cfg.output, "checkpoints", "reinforce_epoch_%02d.pth" % (cfg.test_epoch)
            )
