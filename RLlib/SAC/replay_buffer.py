...
class ReplayMemory:
    ...
    def push(self, state, action, reward, next_state, rnn_state, labels, done):
        ...
        rnn_state_cpu = (
            torch.cat((rnn_state[0].unsqueeze(0), rnn_state[1].unsqueeze(0)), dim=0)
            .cpu()
            .numpy()
            .reshape(-1, rnn_state[0].size(-1))
        )
    ...
    def sample(self, batch_size, device):
        ...
        state, action, reward, next_state, rnn_state, labels, mask = map(
            np.stack, zip(*batch)
        )
        # move data to GPU
        if rnn_state[:, 0] is not None:
            rnn_state = (
                torch.from_numpy(rnn_state[:, 0]).to(device),
                torch.from_numpy(rnn_state[:, 1]).to(device),
            )
...
class ReplayMemoryGPU:
    ...
    def create_buffer(self, gpu_id):
        ...
        reqGPUMem = (
            self.dim_mem * self.batch_size * self.capacity * 4 * 1e-9
        ...
        if freeGPUMem >= reqGPUMem + 1:  # add an extra Gb requirement for memory safety
            buffer = torch.from_numpy(
                np.empty(
                    (self.capacity, self.batch_size, self.dim_mem), dtype=np.float32
                )
            ).to(self.device)
            return 0, buffer
        else:
    ...
    def push(self, state, actions, reward, next_state, rnn_state, labels, done):
        ...
        transition = torch.cat(
            (state, actions, reward, next_state, rnn_state_gpu, labels, done), dim=-1
        )  # (B, 444)
    ...
    def sample(self, batch_size, device):
        ...
        rnn_state = (
            data_batch[:, :, start : start + self.dim_hidden].view(-1, self.dim_hidden),
