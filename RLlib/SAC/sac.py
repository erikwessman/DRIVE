...
class SAC(object):
    def __init__(self, cfg, device=torch.device("cuda")):
        ...
        (
            self.policy_accident,
            self.policy_fixation,
            self.critic,
            self.critic_target,
        ...
        self.policy_acc_optim = Adam(
            self.policy_accident.parameters(), lr=cfg.lr, weight_decay=cfg.weight_decay
        )
        self.policy_att_optim = Adam(
            self.policy_fixation.parameters(), lr=cfg.lr, weight_decay=cfg.weight_decay
        )

        if cfg.arch_type == "rae":
            # the encoder is shared between two actors and critic, weights need to be tied
            self.policy_accident.state_encoder.copy_conv_weights_from(
                self.critic.state_encoder
            )
            self.policy_fixation.state_encoder.copy_conv_weights_from(
                self.critic.state_encoder
            )
            # decoder
            self.decoder = StateDecoder(cfg.dim_latent, self.dim_state).to(
                device=self.device
            )
    ...
    def create_actor_critics(self, cfg):
        ...
        policy_accident = AccidentPolicy(
            dim_state,
            self.dim_action_acc,
            cfg.hidden_size,
            dim_latent=cfg.dim_latent,
            arch_type=cfg.arch_type,
            policy_type=self.type_acc,
        ).to(self.device)
        # create fixation prediction policy
        dim_state = self.dim_state if cfg.arch_type == "rae" else self.dim_state_fix
        policy_fixation = FixationPolicy(
            dim_state,
            self.dim_action_fix,
            cfg.hidden_size,
            dim_latent=cfg.dim_latent,
            arch_type=cfg.arch_type,
            policy_type=self.type_fix,
        ).to(self.device)
    ...
    def select_action(self, state, rnn_state=None, evaluate=False):
        ...
        if evaluate is False:
            action_acc, rnn_state, _, _ = self.policy_accident.sample(
                acc_state, rnn_state
            )
            action_fix, _, _ = self.policy_fixation.sample(fix_state)
        else:
            _, rnn_state, _, action_acc = self.policy_accident.sample(
                acc_state, rnn_state
            )
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_acc_state = (
                next_state_batch.clone() if self.arch_type == "rae" else next_state_max
            )
            next_fix_state = (
                next_state_batch.clone() if self.arch_type == "rae" else next_state_avg
            )
            # inference two policies
            (
                next_acc_state_action,
                _,
                next_acc_state_log_pi,
                _,
            ) = self.policy_accident.sample(next_acc_state, rnn_state_batch)
            (
                next_fix_state_action,
                next_fix_state_log_pi,
                _,
            ) = self.policy_fixation.sample(next_fix_state)
            next_state_action = torch.cat(
                [next_acc_state_action, next_fix_state_action], dim=1
            )
            # interence critics
            ...
            ) - self.alpha * (next_acc_state_log_pi + next_fix_state_log_pi)
            next_q_value = reward_batch + (1 - mask_batch) * self.gamma * (
                min_qf_next_target
            )
        qf1, qf2 = self.critic(
    ...
    def update_actor(self, state_batch, rnn_state_batch, labels_batch):
        ...
        # sampling
        pi_acc, _, log_pi_acc, mean_acc = self.policy_accident.sample(
            acc_state, rnn_state_batch, detach=True
        )
        pi_fix, log_pi_fix, mean_fix = self.policy_fixation.sample(
            fix_state, detach=True
        )
        ...
        cls_loss = exp_loss(
            score_pred, cls_target, curtime_batch, toa_batch
        )  # expoential binary cross entropy
        ...
        fix_pred = scales_to_point(
            fix_pred, self.image_size, self.input_size
        )  # scaling scales to point
        fix_loss = torch.sum(
            torch.pow(
                norm_fix(fix_pred, self.input_size) - norm_fix(fix_gt, self.input_size),
                2,
            ),
            dim=1,
        ).mean()  # (B) [0, sqrt(2)]
        ...
        self.losses.update(
            {
                "policy/total_accident": acc_policy_loss.item(),
    ...
    def update_parameters(self, memory, updates):
        ...
        (
            ...
            labels_batch,
            mask_batch,
        ) = memory.sample(self.batch_size, self.device)
    ...
    def save_models(self, ckpt_dir, cfg, epoch):
        model_dict = {
            "policy_acc_model": self.policy_accident.state_dict(),
    ...
    def load_models(self, ckpt_dir, cfg):
        if cfg.test_epoch == -1:
            ...
            weight_file = os.path.join(
                cfg.output, "checkpoints", "sac_epoch_" + str(cfg.test_epoch) + ".pt"
            )
