...
def setup_dataloader(cfg, num_workers=0, isTraining=False):
    ...
    test_data = DADA2KS(
        cfg.data_path, "testing", interval=cfg.frame_interval, transforms=transform_dict
    )
...
def test_saliency(fusion, rho=None, margin=None, output_dir=None):
    ...
    if not os.path.exists(result_file):
        ...
        # start to test
        with torch.no_grad():
            for i, (video_data, salmap_data, coord_data, data_info) in tqdm(
                ...
                while i_steps < env.max_steps:
                    ...
                    salmaps = np.array(
                        [padding_inv(sal, height, width) for sal in salmaps],
                        dtype=np.float32,
                    )  # (B, 330, 792)
        ...
        dict_result = {
            "SIM": saliency_result[0],
            "CC": saliency_result[1],
            "KL": saliency_result[2],
            "TTA": mTTA,
            "AP": AP,
            "v-AUC": AUC_video,
            "f-AUC": AUC_frame,
