    ...
    for cnt, Th in enumerate(thresolds):
        ...
        for i in range(len(preds_eval)):
            ...
            if all_labels[i] > 0:
                ...
                if len(inds_pos) > 0:
                    time += max(
                        frame_of_accidents[i] - inds_pos[0], 0
                    )  # accumulate #frames (tta) for all pos videos
...
def evaluation_accident(all_pred, all_labels, time_of_accidents, fps=30.0):
    ...
    thresolds[0] += 1e-6
    thresolds[-1] -= 1e-6
    Precision, Recall, Time = compute_metrics(
        preds_eval, all_labels, frame_of_accidents, thresolds
    )
    # when threshold=0.5
    ...
    for i in range(1, len(new_Precision)):
        AP += (
            (new_Precision[i - 1] + new_Precision[i])
            * (new_Recall[i] - new_Recall[i - 1])
            / 2
        )
...
def evaluation_auc_scores(
    ...
    if pos_only:
        all_vid_scores = [
            max(pred[int(toa * FPS) :]) for toa, pred in zip(all_toas, all_pred_scores)
        ]
    else:
        all_vid_scores = [
            max(pred[: int(toa * FPS)]) for toa, pred in zip(all_toas, all_pred_scores)
...
def print_results(
    Epochs, APvid_all, AP_all, mTTA_all, TTA_R80_all, Unc_all, result_dir
):
    result_file = os.path.join(result_dir, "eval_all.txt")
    with open(result_file, "w") as f:
        for e, APvid, AP, mTTA, TTA_R80, Un in zip(
            Epochs, APvid_all, AP_all, mTTA_all, TTA_R80_all, Unc_all
        ):
            f.writelines(
                "Epoch: %s," % (e)
                + " APvid={:.3f}, AP={:.3f}, mTTA={:.3f}, TTA_R80={:.3f}, mAU={:.5f}, mEU={:.5f}\n".format(
                    APvid, AP, mTTA, TTA_R80, Un[0], Un[1]
                )
            )
    f.close()
    ...
    for b in range(vis_batchnum):
        ...
        for n in range(batch_size):
            ...
            if smooth:
                ...
                pred_std_alea = make_interp_spline(xvals, pred_std_alea_reduce)(
                    xvals_new
                )
                pred_std_epis = make_interp_spline(xvals, pred_std_epis_reduce)(
                    xvals_new
                )
            ...
            if toa[n] <= pred_frames.shape[1]:
                plt.axvline(
                    x=toa[n], ymax=1.0, linewidth=3.0, color="r", linestyle="--"
                )
