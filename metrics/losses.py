...
def exp_loss(pred, target, time, toa):
    ...
    penalty = -torch.max(
        torch.zeros_like(toa).to(toa.device, pred.dtype), toa.to(pred.dtype) - time - 1
    )
    ...
    pos_loss = -torch.mul(
        torch.exp(penalty), -nll_loss(torch.log(pred + 1e-6), target_cls)
    )
    # negative example
    # neg_loss = self.ce_loss(pred, target_cls)
    neg_loss = nll_loss(torch.log(pred + 1e-6), target_cls)
    loss = torch.mean(
        torch.add(torch.mul(pos_loss, target[:, 1]), torch.mul(neg_loss, target[:, 0]))
    )
    return loss
...
def fixation_loss(pred_fix, gt_fix, normalize=False, extends=None):
    ...
    if pred_pts.size(0) > 0 and gt_pts.size(0) > 0:
        ...
        # MSE loss
        dist_sq = torch.sum(
            torch.pow(pred_pts.float() - gt_pts.float(), 2), dim=1, keepdim=True
        )
        loss = torch.mean(dist_sq)
    else:
        loss = torch.tensor(0.0).to(pred_fix.device)
    return loss
