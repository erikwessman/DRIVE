...
def auc_judd(s_map, gt):
    ...
    for thresh in thresholds:
        ...
        assert (
            np.max(gt) == 1.0
        ), "something is wrong with ground truth..not discretized properly max value > 1.0"
        assert (
            np.max(s_map) == 1.0
        ), "something is wrong with salience map..not normalized properly max value > 1.0"
...
def infogain(s_map, gt, baseline_map):
    ...
    for i in zip(x, y):
        temp.append(
            np.log2(eps + s_map[i[0], i[1]]) - np.log2(eps + baseline_map[i[0], i[1]])
        )
